...
class ArtDataManager: ObservableObject
{
        ...
        for object in objects {
            ...
            self.getArt(with: url).receive(on: DispatchQueue.main).sink(receiveCompletion: { completion in }, receiveValue: { [weak self] data in
                if let stringURL = data.primaryImage, let name = data.title, let url = URL(string: stringURL), let publicImage = data.isPublicDomain, publicImage {
                    self?.venusArtData.append(MetImageData(objectId: data.objectID, name: name, artistDisplayName: data.artistDisplayName, objectDate: data.objectDate, objectName: data.objectName, url: url, id: UUID().uuidString))
                }
    ...
    func loadMarsArt(objects: [Int])
    {
        for object in objects {
            ...
            self.getArt(with: url).receive(on: DispatchQueue.main).sink(receiveCompletion: { completion in }, receiveValue: { [weak self] data in
                if let stringURL = data.primaryImage, let name = data.title, let url = URL(string: stringURL), let publicImage = data.isPublicDomain, publicImage {
                    self?.marsArtData.append(MetImageData(objectId: data.objectID, name: name, artistDisplayName: data.artistDisplayName, objectDate: data.objectDate, objectName: data.objectName, url: url, id: UUID().uuidString))
                }
            }).store(in: &self.cancellables)
